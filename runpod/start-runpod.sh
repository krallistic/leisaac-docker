...
ENV_JSON=$(
    ...
    DIFFUSION_STEPS="${DIFFUSION_STEPS:-5000}" \
    NUM_WORKERS="${NUM_WORKERS:-4}" \
    RUN_MODE="${RUN_MODE:-train}" \
    CASE="${CASE:-cube_green}" \
    BENCH_INFER_BS="${BENCH_INFER_BS:-1}" \
    BENCH_WARMUP="${BENCH_WARMUP:-5}" \
    BENCH_ITERS="${BENCH_ITERS:-30}" \
    BENCH_INFER_WARMUP="${BENCH_INFER_WARMUP:-10}" \
    BENCH_INFER_ITERS="${BENCH_INFER_ITERS:-50}" \
    python3 -c 'import json,os; ks="GCP_SA_KEY_B64 GCS_BUCKET EXPERIMENT_NAME POLICIES PERCENTS SEEDS EPOCHS BATCH_SIZE LR CONCEPT_WEIGHT CONCEPT_NOISES CONCEPT_GROUP DIFFUSION_LR DIFFUSION_CROP DIFFUSION_STEPS NUM_WORKERS RUN_MODE CASE BENCH_INFER_BS BENCH_WARMUP BENCH_ITERS BENCH_INFER_WARMUP BENCH_INFER_ITERS".split(); print(json.dumps({k: os.environ[k] for k in ks}))'
)
