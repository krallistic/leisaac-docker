#!/bin/bash
# 03-train-concept-act-heads.sh — Train ConceptACT with MLP prediction heads.
#
# Uses the concept-labeled LeRobot datasets (sim/sort_object_with_concepts_*).
# One independent MLP head per concept class is attached to the first token of
# the encoder output.  Each head: Linear → ReLU → Dropout(0.2) → Linear.
# Cross-entropy loss is applied per head. Concept head LR = 10× base LR.
# (prediction_head does not use ClassAwareConceptACTEncoderLayer.)
#
...
#
# Checkpoint naming: concept_act_ph_cw{CW}_lr{LR}_seed{SEED}
#
...
echo "=== ConceptACT training (prediction_head / MLP) ==="
echo "  Datasets      : ${DATASET_LIST}"
echo "  Steps         : ${STEPS}  batch=${BATCH_SIZE}  lr=${LR}"
echo "  Concept weight: ${CONCEPT_WEIGHT}  concept_dim=${CONCEPT_DIM}"
echo "  Concept head LR: 10× base ($(echo "${LR} * 10" | bc -l 2>/dev/null || echo "= ${LR}×10"))"
echo "  Seeds         : ${SEEDS}"
...
echo "=== ConceptACT (prediction_head) training complete ==="
