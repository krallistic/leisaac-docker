#!/bin/bash
# 03-train-concept-act-heads-noise.sh — ConceptACT with MLP prediction heads
# and concept label noise applied during training.
#
# Identical to 03-train-concept-act-heads.sh except that --policy.concept_noise
# is set to CONCEPT_NOISE.  For each sample and each concept class, the true
# one-hot label is replaced with a uniformly random *different* one-hot from
# that class with probability CONCEPT_NOISE.  The one-hot constraint within
# each class is always preserved.  Noise is only applied during training, not
# at eval / inference time.
#
# Checkpoint naming: concept_act_ph_cw{CW}_noise{NOISE}_lr{LR}_seed{SEED}
#
# Key overrideable env vars:
#   CONCEPT_NOISE  per-class flip probability (default: 0.1)
...
#   CONCEPT_DIM    hidden dim of each MLP head (default: 128)
...
CONCEPT_NOISE="${CONCEPT_NOISE:-0.1}"
...
echo "=== ConceptACT training (prediction_head / MLP + concept noise) ==="
echo "  Datasets      : ${DATASET_LIST}"
echo "  Steps         : ${STEPS}  batch=${BATCH_SIZE}  lr=${LR}"
echo "  Concept weight: ${CONCEPT_WEIGHT}  noise=${CONCEPT_NOISE}  concept_dim=${CONCEPT_DIM}"
echo "  Seeds         : ${SEEDS}"
...
for seed in $SEEDS; do
    job="concept_act_ph_cw${CONCEPT_WEIGHT}_noise${CONCEPT_NOISE}_lr${LR}_seed${seed}"
    ...
    docker run --rm \
        ...
        --policy.concept_noise="${CONCEPT_NOISE}" \
        --policy.concept_dim="${CONCEPT_DIM}" \
...
echo "=== ConceptACT (prediction_head + noise) training complete ==="
