#!/bin/bash
# 03-train-concept-act-noise.sh — ConceptACT (transformer_ce + class-aware)
# with concept label noise applied during training.
#
# Identical to 03-train-concept-act.sh except that --policy.concept_noise is
# set to CONCEPT_NOISE.  For each sample and each concept class, the true
...
#
# Checkpoint naming: concept_act_tce_cw{CW}_noise{NOISE}_lr{LR}_seed{SEED}
#
...
echo "=== ConceptACT training (transformer_ce + class-aware + concept noise) ==="
echo "  Datasets      : ${DATASET_LIST}"
echo "  Steps         : ${STEPS}  batch=${BATCH_SIZE}  lr=${LR}"
echo "  Concept weight: ${CONCEPT_WEIGHT}  noise=${CONCEPT_NOISE}"
echo "  Seeds         : ${SEEDS}"
...
for seed in $SEEDS; do
    job="concept_act_tce_cw${CONCEPT_WEIGHT}_noise${CONCEPT_NOISE}_lr${LR}_seed${seed}"
...
echo "=== ConceptACT (transformer_ce + noise) training complete ==="
